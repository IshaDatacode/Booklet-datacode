...
const Card = ({ url, title, subtitle, id }) => {
  ...
  return (
    ...
    onMouseLeave={()=>setEditDisplay(false)}>
      <div className="card bg-imp border border-0 p-2 me-lg-5">
        <div className="dropdown ">
          <span onMouseEnter={handleEllipse} className="btn ellipse-1 fs-4 rounded-2 bg-imp px-2 py-0 mt-0 opacity-75 text-white position-absolute top-0 end-0 " role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="true">
            <FaEllipsisH />
          </span>
          {editDisplay && 
          <div>
          {/*<div className="w-100 h-100 position-fixed z-3 top-0 start-0" onClick={handleModal}></div>*/}
          <ul className="dropdown-menu border-dropdown mb-1 show position-absolute z-12 text-white  start-100 bottom-0 rounded-2 bg-imp text-white opacity-75 p-2" >
